...
// The merchant account is active. Cryptomus displays the asset and network picker
// in its secure invoice, so Wassago never imposes a particular blockchain.
const CRYPTO_PAYMENTS_ACTIVE = true;
      ...
      <div className="lg:grid lg:grid-cols-[1fr_1.1fr] lg:gap-12">
        ...
        <div className="mx-auto max-w-lg lg:mx-0 lg:max-w-none">
            ...
            <div className="mt-3 flex flex-col gap-2 border-t border-ink-100 pt-3">
              ...
              {!item.refillSupported && (
                <p className="text-xs text-ink-500">
                  Sans refill : aucun remplacement automatique n&apos;est inclus après la livraison.
                </p>
              )}
            </div>
          </div>
              ...
              <div className="flex flex-col items-center gap-3 py-4 text-center">
                ...
                {step.created?.trackingToken ? (
                  <div className="mt-3 w-full rounded-xl2 border-2 border-ink-900 bg-amber-50 p-4 text-left">
                    <div className="flex items-center gap-2">
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-ink-900 bg-brand-500">
                        <svg className="h-3.5 w-3.5 text-ink-900" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          {step.kind !== "success" && step.kind !== "pending" && (
            <Link href="/catalogue" className="mt-4 inline-block text-sm text-ink-500 hover:underline">
              ← Retour au catalogue
            </Link>
          )}
        </div>
